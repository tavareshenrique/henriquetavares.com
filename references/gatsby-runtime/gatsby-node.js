...
const { supportedLanguages } = require(path.join(__dirname, '../../i18n'));
...
exports.createPages = ({ graphql, actions }) => {
  ...
  return new Promise((resolve, reject) => {
    const blogPost = path.resolve(repoRoot, 'src/templates/blog-post.js');

    // Create index pages for all supported languages
    Object.keys(supportedLanguages).forEach((langKey) => {
      createPage({
        path: langKey === 'pt-br' ? '/' : `/${langKey}/`,
        component: path.resolve(repoRoot, 'src/templates/blog-index.js'),
        context: {
          langKey,
        },
      });
